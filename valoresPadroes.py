...
def retornaValorResistor (valorOriginal,tolerancia,selecao):
        ...
        multiplicador = 1.0
        valorEscalado= valorOriginal
        
        while valorEscalado >= 9.9:
            valorEscalado = valorEscalado / 10.0
            multiplicador= multiplicador * 10.0

             
        if tolerancia == 10:
            indice= searchsorted(E12, valorEscalado,'right')
            if indice > 0:
                valorInferior= E12[indice-1]*multiplicador
                valorSuperior= E12[indice]*multiplicador    
            else:
                valorInferior= E12[indice]*multiplicador
                valorSuperior= E12[indice+1]*multiplicador
                  
        if tolerancia == 5:
            indice= searchsorted(E24, valorEscalado,'right')
            if indice > 0:
                valorInferior= E24[indice-1]*multiplicador
                valorSuperior= E24[indice]*multiplicador    
            else:
                valorInferior= E24[indice]*multiplicador
                valorSuperior= E24[indice+1]*multiplicador    
                                
        if tolerancia == 1:
        
            indice= searchsorted(E96, valorEscalado,'right')
            if indice > 0:
                valorInferior= E96[indice-1]*multiplicador
                valorSuperior= E96[indice]*multiplicador            
            else:
                valorInferior= E96[indice]*multiplicador
                valorSuperior= E96[indice+1]*multiplicador     
